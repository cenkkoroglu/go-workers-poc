...
func (d *Dispatcher) Dispatch(task Task) {
	...
	if !exists {
		...
		workerID := d.getLeastBusyWorkerID()
		skuInfo = &SKUInfo{
			WorkerID: workerID,
			LastUsed: time.Now(),
		}
...
func (d *Dispatcher) getLeastBusyWorkerID() int {
	leastBusy := 0
	minTasks := len(d.Workers[0].Tasks)

	for i, worker := range d.Workers {
		if len(worker.Tasks) < minTasks {
			leastBusy = i
			minTasks = len(worker.Tasks)
		}
	}
...
func main() {
	...
	// Throughput'u log'a yazdır
	log.Printf("Throughput: %.2f tasks/second", throughput)
}
